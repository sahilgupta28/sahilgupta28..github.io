      ...
      b = {
        ...
        renderGitHubLink: function(a, c, d) {
          return (
            ...
            b.renderLink("https://github.com/" + a, c, d)
          );
        },
        getMessageFor: function(a) {
          var c = a.payload;
          if (
            ((a.repoLink = b.renderGitHubLink(a.repo.name)),
            (a.userGravatar = Mustache.render(
              '<div class="gha-gravatar-user"><img src="{{url}}" class="gha-gravatar-small"></div>',
              { url: a.actor.avatar_url }
            )),
            c.ref &&
              ((a.branch =
                "refs/heads/" === c.ref.substring(0, 11)
                  ? c.ref.substring(11)
                  : c.ref),
              (a.branchLink =
                b.renderGitHubLink(
                  a.repo.name + "/tree/" + a.branch,
                  a.branch
                ) + " at ")),
            c.commits)
          ) {
            var d = c.before + "..." + c.head,
              e = c.commits.length;
            2 === e
              ? (a.commitsMessage = Mustache.render(
                  '<a href="https://github.com/{{repo}}/compare/{{shaDiff}}">View comparison for these 2 commits &raquo;</a>',
                  { repo: a.repo.name, shaDiff: d }
                ))
              : e > 2 &&
                (a.commitsMessage = Mustache.render(
                  '<a href="https://github.com/{{repo}}/compare/{{shaDiff}}">{{length}} more ' +
                    pluralize("commit", e - 2) +
                    " &raquo;</a>",
                  { repo: a.repo.name, shaDiff: d, length: c.size - 2 }
                )),
              c.commits.forEach(function(d, e) {
                return (
                  ...
                  2 > e
                    ? ((d.shaLink = b.renderGitHubLink(
                        a.repo.name + "/commit/" + d.sha,
                        d.sha.substring(0, 6),
                        "gha-sha"
                      )),
                      void (d.committerGravatar = Mustache.render(
                        '<img class="gha-gravatar-commit" src="https://gravatar.com/avatar/{{hash}}?s=30&d=https://a248.e.akamai.net/assets.github.com%2Fimages%2Fgravatars%2Fgravatar-user-420.png" width="16" />',
          ...
          if (c.issue) {
            ...
            (a.issueLink = b.renderLink(c.issue.html_url, f)),
              (a.issueType = "issue"),
              c.issue.pull_request && (a.issueType = "pull request");
          }
          if (c.pull_request) {
            var g = c.pull_request;
            if (
              ((a.pullRequestLink = b.renderLink(
                g.html_url,
                a.repo.name + "#" + g.number
              )),
              (a.mergeMessage = ""),
              c.pull_request.merged)
            ) {
              c.action = "merged";
              var h =
                "{{c}} " +
                pluralize("commit", g.commits) +
                " with {{a}} " +
                pluralize("addition", g.additions) +
                " and {{d}} " +
                pluralize("deletion", g.deletions);
              a.mergeMessage = Mustache.render(
                '<br><small class="gha-message-merge">' + h + "</small>",
                { c: g.commits, a: g.additions, d: g.deletions }
              );
            }
          }
          if (c.comment && c.comment.pull_request_url) {
            var f =
              a.repo.name + "#" + c.comment.pull_request_url.split("/").pop();
            a.pullRequestLink = b.renderLink(c.comment.html_url, f);
          }
          if (
            c.comment &&
            c.comment.body &&
            ((a.comment = c.comment.body),
            a.comment.length > 150 &&
              (a.comment = a.comment.substring(0, 150) + "..."),
            c.comment.html_url && c.comment.commit_id)
          ) {
          ...
          if (
            ("ReleaseEvent" === a.type &&
              ((a.tagLink = b.renderLink(
                c.release.html_url,
                c.release.tag_name
              )),
              (a.zipLink = b.renderLink(
                c.release.zipball_url,
                "Download Source Code (zip)"
              ))),
            "GollumEvent" === a.type)
          ) {
            var i = c.pages[0];
            (a.actionType = i.action),
              (a.message =
                a.actionType.charAt(0).toUpperCase() +
                a.actionType.slice(1) +
                " "),
              (a.message += b.renderGitHubLink(i.html_url, i.title));
          }
          "FollowEvent" === a.type &&
            (a.targetLink = b.renderGitHubLink(c.target.login)),
            "ForkEvent" === a.type &&
              (a.forkLink = b.renderGitHubLink(c.forkee.full_name)),
            "MemberEvent" === a.type &&
              (a.memberLink = b.renderGitHubLink(c.member.login)),
            c.gist &&
              ((a.actionType =
                "fork" === c.action ? c.action + "ed" : c.action + "d"),
              (a.gistLink = b.renderLink(
                c.gist.html_url,
                "gist: " + c.gist.id
              )));
          var j,
            h = Mustache.render(templates[a.type], a),
            k = millisecondsToStr(new Date() - new Date(a.created_at));
          j =
            "CreateEvent" == a.type &&
            ["repository", "branch", "tag"].indexOf(c.ref_type) >= 0
              ? icons[a.type + "_" + c.ref_type]
              : icons[a.type];
          var l = {
            message: h,
            icon: j,
          ...
          return singleLineActivities.indexOf(a.type) > -1
            ? Mustache.render(templates.SingleLineActivity, l)
            : Mustache.render(templates.Activity, l);
        },
        getHeaderHTML: function(a) {
          return (
            a.name
              ? (a.userNameLink = b.renderLink(a.html_url, a.name))
              : (a.withoutName = " without-name"),
            (a.userLink = b.renderLink(a.html_url, a.login)),
            (a.gravatarLink = b.renderLink(
              a.html_url,
              '<img src="' + a.avatar_url + '">'
            )),
            Mustache.render(templates.UserHeader, a)
          );
        },
        getActivityHTML: function(a, c) {
          var d = "",
            e = a.length;
          if ((c && c > e && (c = e), (c = c ? c : e), 0 === c))
        ...
        getOutputFromRequest: function(a, b) {
          ...
          c.open("GET", a),
            c.setRequestHeader("Accept", "application/vnd.github.v3+json"),
        ...
        writeOutput: function(a, c) {
          var d =
            "#" === a.charAt(0)
              ...
              : document.getElementsByClassName(a.substring(1));
    ...
    return (
      (a.feed = function(a) {
        ...
        if (
          ...
          var h =
            "?client_id=" + a.clientId + "&client_secret=" + a.clientSecret;
          (f += h), (g += h);
        }
        if ((a.eventsUrl && (g = a.eventsUrl), "object" == typeof a.templates))
          for (var i in templates)
            "string" == typeof a.templates[i] &&
              (templates[i] = a.templates[i]);
        b.getOutputFromRequest(f, function(f, g) {
          (c = f
            ? Mustache.render(templates.UserNotFound, { username: a.username })
            : b.getHeaderHTML(g)),
            b.renderIfReady(e, c, d);
        }),
          b.getOutputFromRequest(g, function(f, g) {
            if (f)
              d = Mustache.render(templates.EventsNotFound, {
                username: a.username
              });
            else {
...
var templates = {
    ...
    Activity:
      '<div id="{{id}}" class="gha-activity">               <div class="gha-activity-icon"><span class="octicon octicon-{{icon}}"></span></div>               <div class="gha-message"><div class="gha-time">{{{timeString}}}</div>{{{userLink}}} {{{message}}}</div>               <div class="gha-clear"></div>             </div>',
    SingleLineActivity:
      '<div class="gha-activity gha-small">                         <div class="gha-activity-icon"><span class="octicon octicon-{{icon}}"></span></div>                         <div class="gha-message"><div class="gha-time">{{{timeString}}}</div>{{{userLink}}} {{{message}}}</div>                         <div class="gha-clear"></div>                       </div>',
    UserHeader:
      '<div class="gha-header">                 <div class="gha-github-icon"><span class="octicon octicon-mark-github"></span></div>                 <div class="gha-user-info{{withoutName}}">{{{userNameLink}}}<p>{{{userLink}}}</p></div>                 <div class="gha-gravatar">{{{gravatarLink}}}</div>               </div><div class="gha-push"></div>',
    Footer:
    ...
    NoActivity:
      '<div class="gha-info">This user does not have any public activity yet.</div>',
    UserNotFound:
      '<div class="gha-info">User {{username}} wasn\'t found.</div>',
    EventsNotFound:
      '<div class="gha-info">Events for user {{username}} not found.</div>',
    CommitCommentEvent:
      "commented on commit {{{commentLink}}}<br>{{{userGravatar}}}<small>{{comment}}</small>",
    CreateEvent: "created {{payload.ref_type}} {{{branchLink}}}{{{repoLink}}}",
    DeleteEvent:
      "deleted {{payload.ref_type}} {{payload.ref}} at {{{repoLink}}}",
    FollowEvent: "started following {{{targetLink}}}",
    ForkEvent: "forked {{{repoLink}}} to {{{forkLink}}}",
    GistEvent: "{{actionType}} {{{gistLink}}}",
    GollumEvent:
      "{{actionType}} the {{{repoLink}}} wiki<br>{{{userGravatar}}}<small>{{{message}}}</small>",
    IssueCommentEvent:
      "commented on {{issueType}} {{{issueLink}}}<br>{{{userGravatar}}}<small>{{comment}}</small>",
    IssuesEvent:
      "{{payload.action}} issue {{{issueLink}}}<br>{{{userGravatar}}}<small>{{payload.issue.title}}</small>",
    MemberEvent: "added {{{memberLink}}} to {{{repoLink}}}",
    PublicEvent: "open sourced {{{repoLink}}}",
    PullRequestEvent:
      "{{payload.action}} pull request {{{pullRequestLink}}}<br>{{{userGravatar}}}<small>{{payload.pull_request.title}}</small>{{{mergeMessage}}}",
    PullRequestReviewCommentEvent:
      "commented on pull request {{{pullRequestLink}}}<br>{{{userGravatar}}}<small>{{comment}}</small>",
    PushEvent:
      'pushed to {{{branchLink}}}{{{repoLink}}}<br>                <ul class="gha-commits">{{#payload.commits}}<li><small>{{{committerGravatar}}} {{{shaLink}}} {{message}}</small></li>{{/payload.commits}}</ul>                <small class="gha-message-commits">{{{commitsMessage}}}</small>',
    ReleaseEvent:
      'released {{{tagLink}}} at {{{repoLink}}}<br>{{{userGravatar}}}<small><span class="octicon octicon-cloud-download"></span>  {{{zipLink}}}</small>',
